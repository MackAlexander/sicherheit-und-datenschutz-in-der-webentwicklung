...
## Beschreibung
Dieses Modul fügt ein Admin-Menü im Wordpress-Admin-Bereich hinzu, indem Protokolleinträge und Sperrungen eingesehen werden können.
>
...
## Funktionen
> ### `add_menu()`
> **Ausführendes Ereignis:** Wordpress Hook [admin_menu](https://developer.wordpress.org/reference/hooks/admin_menu/) 
> 
> **Beschreibung:**
> Diese Funktion ruft die Funktion `add_management_page()` auf und fügt einen neuen Menüpunkt namens `WPProtect` im Werkzeuge-Menü des WordPress-Admin-Bereichs hinzu. 
...
> 
> **Beschreibung:**
> Nach einer erfolgreichen Rechteprüfung mittels `current_user_can()` und der Berechtigung `manage_options` wird eine Navigationsleiste mit zwei Tabs angezeigt.
> 
...
> 
> Der zweite Tab zeigt aktuelle `Sperrungen` an. Wird dieser ausgewählt, wird die Funktion `render_bans()` aufgerufen.

> ### `render_access_log()`
> **Ausführendes Ereignis:** Der `WPProtect` Menüpunkt wird geöffnet oder der Tab `Access Log` wird ausgewählt.
> 
> **Beschreibung:**
...
> Ist diese Zahl größer als die definierte Anzahl an Einträgen pro Seite, wird durch Teilen der `Einträge pro Seite` durch die `Gesamteinträge` die Anzahl an Seiten an Protokolleinträgen berechnet.
>
...
> Ist diese Zahl größer als die definierte Anzahl an Einträgen pro Seite, wird durch Teilen der `Einträge pro Seite` durch die `Gesamteinträge` die Anzahl an Seiten an Sperrungen berechnet.
>
...
> Unter der Tabelle befinden sich Links, um die verschiedene Seiten an Sperrungen durchzublättern, sofern es mehr Einträge gibt, als auf einer Seite angezeigt werden sollen.
> Dabei werden immer nur die Sperrungen für die jeweilige Seite geladen, um den Ressourcenverbrauch so gering wie möglich zu halten. 
